...
public class ServicePagosMasivosController implements Controller {
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		metodos.put("pagosMasivos", 0);

		String metodo = request.getParameter("metodo");

		switch (metodos.get(metodo)) {
		case 0:
			outParameterA = registrarPagoMasivos(request, response, user);
			break;
		}
		...
		ModelAndView modelAndView = new ModelAndView("/servicios/servicio_pagos_masivos", "mapa", respuesta);
		return modelAndView;

	}

	private Map<String, Object> registrarPagoMasivos(HttpServletRequest request, HttpServletResponse response, Usuario user)
		...
		if(nombreArchivo.contains(".csv")) {
			// INSERTAR RENDICION
			Map<String, Object> rend = oneClickDao.cargarPagoMasivoOC(date, nombreArchivo, user.getId(),
